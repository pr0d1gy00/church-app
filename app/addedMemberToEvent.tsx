				...
				<View className="py-4">
					...
					{selectedEventToSend ? (
						<View className="py-4">
							...
							<FlatList
								data={allUsers?.users.filter(
									(user) =>
										user.id !==
										eventDataById?.event.subscriptions.find(
											(member) =>
												member.userId ===
												user.id
										)?.userId
								...
								renderItem={({ item: user }) => (
									...
									>
										...
										/>
										<Text className="text-[#0b1c0c] text-lg">
											{user.name}
										</Text>
									</View>
								)}
						...
						<Text className="text-red-500 text-center my-4">
							Selecciona un evento para agregar miembros
						</Text>
					)}
				</View>
				<Pressable
					onPress={() => handleSubmitAddMemberToEvent()}
