				...
				<View className="w-[90%] py-4">
					...
					<View className="py-4">
						...
						<View className="bg-[#f2f2f2]  rounded-lg">
							...
							>
								{allCells?.cells.filter( fil => fil.id === parseInt(id as string)).map((cell) => (
									<Picker.Item
										key={cell.id}
										label={cell.name}
										value={cell.id}
						...
						<View className="py-4">
							...
							<FlatList
								data={allUsers?.users.filter(user=>user.id !==
									cellByIdData?.cell.members.find(member=>member.userId === user.id)?.userId
								)}
					...
					<Pressable onPress={()=>handeSubmitaddMemberToCell()}
